...
from . import cython_loops_double
...
class HPF:
    ...
    def __init__(self, k=30, a=0.3, a_prime=0.3, b_prime=1.0,
                 ...
                 approx_rte=False, cut_extreme_initial = 0.1):
        ...
        if check_every is not None:
            ...
            if stop_crit != 'maxiter':
                raise ValueError(
                    "If 'stop_crit' is not 'maxiter', must input after how many iterations to calculate it.")
            check_every = 0
        ...
        self.approx_rte = bool(approx_rte)
        self.cut_extreme_initial = cut_extreme_initial
    ...
    def _process_data(self, input_df):
        ...
        cython_loops = cython_loops_double
    ...
    def _fit(self):
        ...
        self.niter, temp, self.train_llk = cython_loops.fit_hpf(
            ...
            cython_loops.cast_int(self.approx_rte),
            self.cut_extreme_initial
        )
    ...
    def predict_factors(self, counts_df, maxiter=10, ncores=1, random_seed=1, stop_thr=1e-3, return_all=False):
        ...
        ncores, random_seed, stop_thr, maxiter = self._check_input_predict_factors(ncores, random_seed, stop_thr,
                                                                                   maxiter)
            ...
            if update_existing:
                ...
                if self.keep_all_objs:
                    ...
                    self.k_rte[user_id] = self.a_prime / self.b_prime + \
                                          (temp[0].reshape((1, -1)) / temp[1].reshape((1, -1))).sum(axis=1,
                                                                                                    keepdims=True)
            else:
                if self.reindex:
    ...
    def predict(self, user, item):
        ...
        if user.shape[0] == 1:
            ...
            if nan_entries.sum() == 0:
                ...
                out[~nan_entries] = cython_loops.predict_arr(self.Theta, self.Beta,
                                                             non_na_user.astype(cython_loops.obj_ind_type),
                                                             non_na_item.astype(cython_loops.obj_ind_type), self.ncores)
